 ...
 *
 * TRANSPORT: `index.html` already loads GTM (`GTM-MH6MH7F6`) and configures
 * gtag for `G-H4KQL6J5NQ`, so GA4 is already tagged before this bundle runs.
 * These events ride that tag rather than initialising a second one. Loading
 * the Firebase Analytics SDK as well would issue a competing `config` on the
 * same measurement ID and double every number in the property.
 *
 * NOT HERE — `page_view`. GA4 Enhanced Measurement sends it on load and on
 * every history change, which React Router triggers on navigation. (Confirmed
 * in Realtime: the top page title accounts for 76% of views, so the other
 * routes are being recorded already.) Sending our own would double-count.
 * `components/Analytics.tsx` still sets `document.title` per route, because
 * that is the title GA4 reads when it fires.
 *
 ...
 *
 * Deliberately not gated to production. The tag in `index.html` reports from
...
export function track<K extends keyof TrackedEvents>(
  ...
  try {
    ...
    // Blocked, or the tag was removed from index.html. Not our problem.
  }
}
