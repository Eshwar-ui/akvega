...
export default function ContactForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // The copy already distinguishes an endpoint that is not wired up yet
      // from a connection that died, so the reported reason splits the same
      // way — filing both as network would hide a deploy that never landed.
      const unavailable =
        error instanceof Error && error.message === 'CONTACT_ENDPOINT_UNAVAILABLE'

      track('contact_form_error', {
        form_name: 'contact',
        reason: unavailable ? 'endpoint_unavailable' : 'network_or_invalid_response',
      })
